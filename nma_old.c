...
void Compute_Hessian_INM (float **Hessian, int N,
			  struct interaction *Int_list, int N_int,
			  atom *atoms);
		      ...
		      float *B_CA, float *Cart_collectivity)
{
  ...
  f_Diagonalize(N, Hessian, eigen_value, eigen_vector, -1);
...
void Compute_Hessian_INM (float **Hessian, int N,
			  ...
			  atom *atoms)
{
  int i, j, n; double *Hess_diag=malloc(N*sizeof(double));

  for(i=0; i<N; i++)for(j=0; j<N; j++)Hessian[i][j]=0;
  for(i=0; i<N; i++)Hess_diag[i]=0;
  ...
  for(n=0; n<N_int; n++){
    ...
    Hess_diag[i] +=h; Hess_diag[j] +=h;
  }
  for(i=0; i<N; i++)Hessian[i][i]=Hess_diag[i];
  free(Hess_diag);
  return;
}
...
void Compute_Hessian_TNM(float **Hessian,  // Output
			 ...
			 float K_BA, float K_BL)
{
  ...
  double **H_mat=Allocate_mat2_d(N_modes, N_modes);
  ...
  if(T_sqrt_inv){
	...
	for(i=0; i<N_axes; i++){
	  if(axes[i].type=='f'){
	    if(K_PHI)H_Phi+=tk[i]*tl[i];
	  }else if(axes[i].type=='p'){
	    if(K_PSI)H_Psi+=tk[i]*tl[i];
	  }else if(axes[i].type=='l'){
	    if(K_BL)H_BL+=tk[i]*tl[i];
	  }else if(axes[i].type=='a'){
	    if(K_BA)H_BA+=tk[i]*tl[i];
	  }else if(axes[i].type=='t'){
	    if(K_PHI)H_BT+=tk[i]*tl[i];
	  }else if(axes[i].type=='o'){
	    if(K_OMEGA)H_Ome+=tk[i]*tl[i];
	  }else if(axes[i].type=='s'){
	    if(K_CHI)H_Chi+=tk[i]*tl[i];
	  }
	}
	H_mat[l][k]+=K_BL*H_BL+K_BA*H_BA+K_PHI*H_BT+
	  K_PHI*H_Phi+K_PSI*H_Psi+K_OMEGA*H_Ome+K_CHI*H_Chi;
      }
    }
		  ...
		  int ini_atom, int natoms)
{
  ...
  int N_ref=Select_atoms(Ref->atom_num+ini_ref, atoms+ini_atom, natoms, SEL);
  Ref->N_ref=N_ref;
  Ref->N_cart=3*N_ref;
  printf("%d atoms out of %d selected for kinetic energy (%s)\n",
	 N_ref, natoms, SEL);
			     ...
			     char type, float *Lambda)
{
  ...
  /* Formula: sum_a J_ia Delta_theta_a ~ sqrt(m_i) Delta_r_i  where:
  // J is the jacobian matrix dr_int_i/d_theta_a
  // M_i is the diagonal mass matrix 
     The fit is performed through the ridge regression approach
  // Lamda=0: ==> L^t Delta_theta = L^{-1} M Delta_r = Jtilde M Delta_r
  // L=T_sqrt (lower diagonal matrix if obtained from Cholevsky decomposition)
  // T=JMJ^t = LL^t
  //
  */
  
  int Npar=J->N_axes, Nsam=J->N_cart, N=Nsam/3, a, i;
  float sm[N]; for(i=0; i<N; i++)sm[i]=sqrt(Ref.mass_coord[i*3]);

  float **X=malloc(Nsam*sizeof(float *));
  float D_out[Npar], Y[Nsam]; int k=0;
  for(int i=0; i<N; i++){
    for(int j=0; j<3; j++){
      Y[k]=sm[i]*D->Cart[k];
      float *xk=malloc(Npar*sizeof(float)); X[k]=xk;
      for(a=0; a<Npar; a++)xk[a]=sm[i]*J->Jacobian_ar[a][k];
      k++;
    }
  }
  ...
  char out[200]; sprintf(out, "%s_ridge_regression.dat", nameout);
  struct ridge_fit fit; fit.A=malloc(Npar*sizeof(float));
  fit.Lambda=*Lambda;
  float y_pred[Nsam];
  float r=
    Ridge_regression(&fit, y_pred, D_out, out, X, Y, Nsam, Npar, type);
...
void Compute_MW(struct Tors *D, struct Jacobian *J){
  ...
  for(a=0; a<N; a++){
    ...
    if(J->T_sqrt_inv){ // T_sqrt is not lower diagonal! DMW =L^t Dtheta
      for(b=0; b<N; b++)W+=J->T_sqrt[b][a]*D->Tors[b];
    }else{  // b<a
      for(b=a; b<N; b++)W+=J->T_sqrt[b][a]*D->Tors[b];
    }
    D->MW_Tors[a]=W;
  }
}
...
int Select_atoms(int *atom_num,  atom *atoms, int N_atoms, char *SEL)
{
  ...
  for(i_atom=0; i_atom<N_atoms; i_atom++){
    if((atom1->ali>=0)&&(Selection(atom1->name, atom1, SEL))){
      atom_num[n]=i_atom; n++;
    }
    atom1++;
  }
		       ...
		       struct residue *res, char *nameout1)
{
  // Print in file
  char namefile[200]="";
  sprintf(namefile, "%s_tors_fluct.dat", nameout1);
  printf("Writing %s\n", namefile);
...
float Compute_correlation(float **Cart_mode, float *contr2fluct,
			  int N_modes, int i1, int i2)
{
  int ia, k1=3*i1, k2=3*i2, j;
  double corr=0; float *r1, *r2;
  for(ia=0; ia< N_modes; ia++){
    if(contr2fluct[ia]==0)continue;
    r1=Cart_mode[ia]+k1;
    r2=Cart_mode[ia]+k2;
    for(j=0; j<3; j++){corr += (*r1)*(*r2)*contr2fluct[ia]; r1++; r2++;}
  }
  return(corr);
}
		       ...
		       float *dof, char ridge)
{
  ...
  fprintf(file_out, "# aa_pos_chain pred_internal");
  if(B_pred_all && B_exp) fprintf(file_out, " pred_all exp");
  fprintf(file_out, "\n");
  for(i=0; i<N; i++){
    atom1=atoms+atom_ref[i]; struct residue *r=seq+atom1->res;
    int j;
    for(j=sizeof(r->pdbres)-1; j>=0; j--)
      if(r->pdbres[j]==' '){r->pdbres[j]='\0';}else{break;}
    for(j=0; j<sizeof(r->pdbres); j++)if(r->pdbres[j]!=' ')break;
    char name[20]; sprintf(name, "%c%s%c", r->amm, r->pdbres+j, r->chain);
    fprintf(file_out, "%s\t%7.3f", name, B_TNM[i]);
    if(B_pred_all && B_exp)
...
void Compute_force(struct Tors *Force, float *cc, struct Normal_Mode NM)
{
  ...
  for(a=0; a<NM.N_relevant; a++){
    if((NM.select[a]==0)||(NM.contr2fluct[a]==0))continue;
...
void Torsional_force(struct Tors Force, struct Tors *Diff, float **Hessian,
