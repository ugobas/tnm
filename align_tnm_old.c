...
{
  float seqid_min=0.3; // seqid_max=0.9;
  ...
  *Nmut=0;
  ...
  //int match[Nchain1][Nchain2], i,j;
  //for(i=0; i<Nchain1; i++)for(j=0; j<Nchain2; j++)match[i][j]=0;

  int n2max=Max_length(chains2, Nchain2);

  //Match_chains_by_sequence
  int NMUT=1000;
  int nres1=0, nres2=0;
  int ichain, jchain, i;
  for(ichain=0; ichain< Nchain1; ichain++){
    ...
    // Print alignment
    printf("Protein_1: ");
    for(i=0; i<nali_opt; i++)printf("%c", ali1_opt[i]);
    printf("\nProtein_2: ");
    for(i=0; i<nali_opt; i++)printf("%c", ali2_opt[i]);
    printf("\n");
