...
struct bond *Find_previous(struct bond *bond, int n, int N_atoms,
			   struct residue *seq);
void Make_frame(struct bond *bond, float *pz);
...
struct bond *Set_bonds_topology(int N_atoms, atom *atoms, struct residue *seq)
{
  ...
  for(i=0; i<N_atoms; i++){
    ...
    struct bond *bond=bonds+n, *previous=NULL;
    ...
    if(atom->chain!=chain){
      ...
      previous=bonds+lastchain;
    }else if(n){
      previous=Find_previous(bond, n, N_atoms, seq+bond->atom->res);
      if(previous==NULL){
...
int Set_bonds_measure(struct bond *bonds, int N_atoms, atom *atoms)
{
  ...
  for(i=0; i<N_atoms; i++){
    ...
    float r_sin_theta= sqrt(dr_dr-bond->r_cos_theta*bond->r_cos_theta);
...
void Build_up(struct bond *bonds, int N_atoms, float *d_phi, int N_axes)
{
  ...
  for(i=1; i<N_atoms; i++){
    ...
    double dr_dr=Scalar_product_3(bond->dr, bond->dr);
    bond->l=sqrt(dr_dr);
    for(j=0; j<3; j++){
      bond->dz[j]=bond->dr[j]/bond->l;
    }
