...
int Set_dof(struct axe **axe, int *num_dof, 
	    char type, struct bond *bond, int first_atom,
...
void Axe_prot_side(struct axe *axe_side, int *N_side, struct side_chain *sc,
		   struct bond *bond, int ibond,
		   atom *atoms, int natoms, int *refatom);
			   ...
			   int PSI)       // Are psi angles used?
{
  ...
  N_axe_max+=3*Ngap;
  printf("Expected number of dofs: %d\n", N_axe_max); //exit(8);
  ...
  */
  ...
  for(ibond=2; ibond<natoms; ibond++){
	...
	type=Axe_prot_main(bond, last_r);
      }else{
	type=Axe_nuc_main(bond, last_r);
      }
      if(type=='s'){
	if((SCHAIN)&&(bond->atom->res>0))
	  Axe_prot_side(axe_side, &N_side, &sc, bond, ibond,
	...
	Set_dof(&axe, &N_axes, 'a', bond, first_atom, &ndof,
		&oldref, refatom, natoms, last_a, last_ref);
	first_atom=(Next_bond(bond, ibond, natoms))->i_atom;
	Set_dof(&axe, &N_axes, 't', bond, first_atom, &ndof,
		&oldref, refatom, natoms, last_a, last_ref);
	continue;
      }
      if(type==NONE){continue;}
      struct bond *next_bond=Next_bond(bond, ibond, natoms);
      first_atom=next_bond->i_atom;
      if(type=='o'){ // omega angle: set for transitions with D_omega>thr
	if((OMEGA>0)||((OMEGA<0)&&(next_bond->d_phi>D_omega_thr))){
	  Set_dof(&axe, &N_axes, type, bond, first_atom, &ndof,
		  &oldref, refatom, natoms, last_a, last_ref);
	  if(OMEGA<0){
	    atom *atom=(bond+1)->atom;
	    printf("Omega %s %c%d d=%d\n", atom->name,
		   atom->aa, atom->res, next_bond->d_phi);
	  }
	}
      }else{
  ...
  *nskip=0; int nskip_side=0;
  ...
  for(a=0; a<*nmain; a++){
    ...
    if(atoms[axe->first_atom].chain != ichain){
      ichain=atoms[axe->first_atom].chain;
  ...
  if(DEBUG)printf("Degrees of freedom associated to bonds:\n");
  for(i=0; i<N_axes; i++){
    axe=axes+i;
    if(DEBUG)
      printf("%c %s%c ", axe->type,axe->bond->atom->name,
	     res[axe->bond->atom->res].amm);
    if(axe->type=='l'){
      axe->bond->len=i;
    }else if(axe->type=='a'){
      axe->bond->angle=i;
      ...
      // axe->bond->torsion=i;
      // Find all bonds whose previous bond coincides with axe->bond
      struct bond *bond=axe->bond+1;
      int res_max=bond->atom->res+1;
      while((bond->atom)&&(bond->atom->res <= res_max)){
	if(bond->previous==axe->bond){
	  bond->torsion=i;
	  if(DEBUG)printf("%s%c ", bond->atom->name,res[bond->atom->res].amm);
	}
	bond++;
      }
    }
  ...
  if(0){
    ...
    for(i=0; i<N_axes; i++){
      printf("%c  ", axe->type);
      atom *a=axe->bond->previous->atom;
      printf(" %s%d", a->name, a->res);
      a=axe->bond->atom; printf(" %s%d", a->name, a->res);
  ...
  if(0){
    char name_out[80]="axes.dat";
    FILE *file_out=fopen(name_out, "w");
    struct axe *axe=axes;
    for(i=0; i<N_axes; i++){
      fprintf(file_out, "%d %c atoms: %d.%s - %d.%s ref: %d.%s - %d.%s ",
	      i, axe->type, axe->first_atom, atoms[axe->first_atom].name, 
	      axe->last_atom, atoms[axe->last_atom].name,
	      axe->first_kin, atoms[axe->first_kin].name,
	      axe->last_kin, atoms[axe->last_kin].name);
      fprintf(file_out, "axe: %s -%s \n",
  ...
  printf("Chain %c-", chains[ichain].label);
  for(i=0; i<N_axes; i++){
    axe=axes+i;
    if(atoms[axe->first_atom].chain>ichain){
      ichain=atoms[axe->first_atom].chain; ig=0;
      printf("-NewChain %c-", chains[ichain].label);
	    ...
	    int last_atom, int last_ref)
{
  ...
  //printf("%c%s-%s", type, bond->previous->atom->name, bond->atom->name);
  (*axe)->type=type; //l,a,t=(p,f,o)(1,2,3,4,5,6)(s)
  (*axe)->bond=bond;
  // rotation axe associated to dof
  /*if((type=='l')||(type=='a')){(*axe)->bond=bond;}
    else{(*axe)->bond=bond->previous;} // Torsion */
		    ...
		    int MIN_INT)
{
  ...
  test_ref:
    move_atom=(Next_bond(bond, ibond, natoms))->i_atom;
    ...
    if(Set_dof(&axe, &N_axes, type[0], bond, move_atom, &ndof,
  ...
  for(i=0; i<natoms; i++){
    ...
    if((bond->previous!=NULL)&&(axes[n].bond==bond->previous)){
		   ...
		   atom *atoms, int natoms, int *refatom)
{
  ...
  int first_atom=(Next_bond(bond, ibond, natoms))->i_atom;
  if(Set_dof(&axe, N_side, 's', bond, first_atom, &sc->ndof,
...
int Last_axe(int *aa, struct axe *axes, int ini, int n, int iatom)
{
  ...
  if(a==0)return(-1);
  if(a==n){a--; axe--;}
  if(axe->first_atom>iatom && (axe-1)->first_atom<=iatom){a--; axe--;}
  if(a<ini){*aa=a;}else{*aa=ini;}
  if((axe->first_atom<=iatom))return(a); //&&(axe->last_atom>=i)
  printf("WARNING, last_axe<0: atom %d axe %d n= %d first %d last %d\n",
	 iatom, a, n, axe->first_atom, axe->last_atom); //exit(8);
  return(-1);
}

struct bond *Next_bond(struct bond *bond, int ibond, int nbond){
  for(int i=1; i<(nbond-ibond); i++){
